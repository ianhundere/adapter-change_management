    ...
    getRecord(callback) {
        ...
        ServiceNowConnector.get((data, error) => {
            ...
            if (listKeys.length > 0) {
                if (listKeys.includes(findBody)) {
                    console.log('Object contains body key');
                    data.body = {
                        change_ticket_number: data.body.number,
                        active: data.body.active,
                        priority: data.body.priority,
                        description: data.body.description,
                        work_start: data.body.work_start,
                        work_end: data.body.work_end,
                        change_ticket_key: data.body.sys_id,
                    };
                    data.body = JSON.parse(data.body);
                    return callback(data.body);
                } else {
                    console.log("Object doesn't contain body key.");
                }
            }
        });
    }
