// Update this constant with your ServiceNow credentials
  const url = 'https://dev74924.service-now.com';
  const username = 'admin';
  const password = 'Service2020';
...
function isHibernating(response) {
  return (
    response.body.includes('Hibernating Instance') &&
...
function sendRequest(callOptions, callback) {
  ...
  const { serviceNowTable, query, method, ...rest } = callOptions;

  let uri;
  if (query) uri = constructUri(serviceNowTable, query);
  else uri = constructUri(serviceNowTable);
  /**
